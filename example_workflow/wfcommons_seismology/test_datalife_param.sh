...
LD_PRELOAD=$DATALIFE_LIB_PATH \
    python seism_workflow.py 2>&1 | tee tazer_test.log
# datalife-run python seism_workflow.py 2>&1 | tee tazer_test.log
t2=$(date +%s%3N)
echo "Time taken[datalife-run]: $((t2-t1)) milliseconds"

rm -rf test-seis.json
sleep 5
...
sudo /sbin/sysctl vm.drop_caches=3
t3=$(date +%s%3N)
python seism_workflow.py 2>&1 | tee -a tazer_test.log
t4=$(date +%s%3N)
