...
RUN_IOR_MEM_DISK_TEST(){
    ...
    for tsize in 64 1k 4k 1m; do
        for trial in 1 2 3; do
            ...
            test_file="$FS/${test_name}.bin"
            ior -a POSIX -w -r -t $tsize -s 4096 -e -F -o $test_file -O summaryFormat=JSON -O summaryFile=${test_name}.json
            sudo drop_caches
            sleep 5
        done
    done
    ...
    for tsize in 64 1k 4k 1m; do
        for trial in 1 2 3; do
            ...
            test_file="$FS/${test_name}.bin"
            ior -a POSIX -w -r -t $tsize -s 16384 -e -F -o $test_file -O summaryFormat=JSON -O summaryFile=${test_name}.json
            sudo drop_caches
            sleep 5
        done
    done
    ...
    for tsize in 1k 4k 1m; do
        for trial in 1 2 3; do
            ...
            test_file="$FS/${test_name}.bin"
            ior -a POSIX -w -r -t $tsize -s 32768 -e -F -o $test_file -O summaryFormat=JSON -O summaryFile=${test_name}.json
            sudo drop_caches
            sleep 5
        done
    done
    ...
    for tsize in 1k 4k 1m; do
        for trial in 1 2 3; do
            ...
            test_file="$FS/${test_name}.bin"
            ior -a POSIX -w -r -t $tsize -s 65536 -e -F -o $test_file -O summaryFormat=JSON -O summaryFile=${test_name}.json
            sudo drop_caches
            sleep 5
        done
    done
