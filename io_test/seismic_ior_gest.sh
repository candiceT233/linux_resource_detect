    ...
    for FS in "$TEST_DIR"; do
        ...
        for tsize in 64 1k 2k 4k; do
            ...
            for trial in 2 3; do # {1..3}
                ...
                echo "Writing File:"
                ior -a POSIX -w -t $tsize -b 512m -s 10 -e -F -k -e -useO_DIRECT -o $test_file -O summaryFormat=JSON -O summaryFile=w_${test_name}.json

                `$DROP_CACHE_CMD`
                sleep 5

                echo "Reading File:"
                ior -a POSIX -r -t $tsize -b 512m -s 10 -e -F -E -k -e -useO_DIRECT -o $test_file -O summaryFormat=JSON -O summaryFile=r_${test_name}.json
