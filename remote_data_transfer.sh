...
PRINT_USAGE () {
    ...
    echo "Options:"
    echo "  -u, --user <user name>: User name to check (default: current user)"
    echo "  -s, --server <remote server>: Remote server name"
    echo "  -i, --identity <identity file>: Identity file to use for SSH connection"
    echo "  -o, --output <output file>: File with all detected user storage paths"
    echo "  -d, --data <data file>: Path to the file containing absolute user data file paths"
    echo "  -m, --mode <mode>: Mode (0 - do not restore user data, 1 - restore user data, default: 1 for testing)"
    echo "  -log, --log-level <log level>: Log level (0 - minimal log, 1 - full log, default: 1)"
